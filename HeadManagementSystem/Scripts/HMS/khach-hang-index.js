...
HMS.namespace('KhachHang');
HMS.KhachHang = function () {
    var Global = {
        UrlAction: {
    ...
    this.Edit = function (Id) {
        $('#btSave').show();
        $('#btAdd').hide();
        Global.Data.selectedObj = $.map(Global.Data.KhachHangXe, function (item, i) {
            if (item.Id == Id)
                return item;
        })[0];
    ...
    this.Init = function () {
        ...
        $('#tinh-thanhpho').empty();
        $('#quan-huyen').empty();
        // $('#tinh-thanhpho').append('<option selected disabled value="">Chọn ...</option>');
        $.each(Global.Data.TPho, function (index, item) {
    ...
    var RegisterEvent = function () {
        $('#tinh-thanhpho').change(function () {
            $('#quan-huyen').selectpicker('destroy');
            $('#quan-huyen').empty();
            $.each(Global.Data.TPho, function (i, item) {
                if (item.name == $('#tinh-thanhpho').val().trim()) {
                    $.each(item.districts, function (i, subitem) {
                        $('#quan-huyen').append('<option value="' + subitem + '" ' + (Global.Data.HuyenValue == subitem ? 'selected' : '') + '>' + subitem + '</option>');
                    });
                }
            });
        ...
        $("#KhachHang-type").on("changed.bs.select",
        ...
        $.ajax({
            ...
            success: function (objs) {
                ...
                if (Global.Data.table != null) {
                    ...
                    Global.Data.KhachHangXe.length = 0;
                }
                Global.Data.KhachHangXe = objs;
                InitTable(objs);
            }
        });
    }
